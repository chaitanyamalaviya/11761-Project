...
if __name__ == '__main__':
	trainingData = getTrainingData("trainingSet.txt")
	labels = getTrainingLabels("trainingSetLabels.txt")
	...
	realDocs = filter(lambda x: x[0] == 1, trainingData)
	fakeDocs = filter(lambda x: x[0] == 0, trainingData)

	print "Real Docs..."
	realDocsScore = run(vocabDict, m, realDocs)

	print ""
	print "Fake Docs..."
	fakeDocsScore = run(vocabDict, m, fakeDocs)

	print "Real Docs Average"
	print realDocsScore

	print "Fake Docs Average"
	print fakeDocsScore
