...
class GenericStanfordParser(ParserI):
    ...
    def _parse_trees_output_PCFG(self, output_):
        scores = []
        for line in output_.splitlines(False):
            matchObj = re.match( r'# Parse 1 with score (.*)$', line, re.M|re.I)
            if matchObj:
                scores.append(matchObj.group(1))
        return scores
    ...
    def raw_parse_sents_PCFG(self, sentences, verbose=False):
        """                                                                                                                                             
        Use StanfordParser to parse multiple sentences. Takes multiple sentences as a                                                                   
        list of strings.                                                                                                                                
        Each sentence will be automatically tokenized and tagged by the Stanford Parser.                                                                
        ...
        return self._parse_trees_output_PCFG(self._execute(cmd, '\n'.join(sentences), verbose))
